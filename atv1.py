perfis = {
  "caredenciais_do_SOC":      {"schema_fixo":True,  "precisa_acid":True,  "escala_horizontal":False, "tolera_atraso_de_consistencia":False, "dado_sensivel":True},
...
def escolher_banco(perfil):
    ...
    estrutura = perfil["schema_fixo"]
    transacao = perfil["precisa_acid"]

    if estrutura and transacao:
        return "MySQL"
...
def escolher_justificativa(perfil):

    return JUSTIFICATIVAS[assinatura(perfil)]


def escolher_risco_owasp(perfil):

    return RISCOS_OWASP[assinatura(perfil)]
...
def recomendar(perfil):
    ...
    return {
        ...
        "justificativa": escolher_justificativa(perfil),
        "risco_owasp": escolher_risco_owasp(perfil),
    }
