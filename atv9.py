...
def analisar():
    ...
    referencia_normal = np.array([[0.5, 0.00, 2], [1.0, 0.02, 2], [2.0, 0.05, 3], [3.0, 0.10, 3], [5.0, 0.08, 4]], dtype=float)
    ...
    for dado, previsao in zip(dados, previsoes):
        ...
        if anomalia:
            bloqueios.update_one({"ip": dado["ip"]}, {"$set": {"bloqueado_ate": agora + timedelta(seconds=60)}}, upsert=True)
        relatorio.append({**dado, "classificacao": "ANOMALIA" if anomalia else "normal", "bloqueado": anomalia})
...
if __name__ == "__main__":
    if "--test" in sys.argv:
        ...
        # Bloquear por anomalia pode gerar falso positivo e derrubar um usuário legítimo.
        # Por isso, o modelo deve ser monitorado, auditado e combinado com revisão ou regras de segurança.
