"""Exercício 8 — classificador de risco servido por API.

Dependências: pip install flask pymongo numpy scikit-learn
...
def treinar_modelo():
    ...
    normais = np.column_stack([
        rng.integers(0, 4, 400), rng.integers(1, 5, 400), rng.integers(500, 18000, 400), rng.integers(7, 23, 400)
    ])
    riscosos = np.column_stack([
        rng.integers(8, 31, 400), rng.integers(6, 26, 400), rng.integers(50000, 220000, 400), rng.integers(0, 7, 400)
    ])
    entradas = np.vstack([normais, riscosos])
    rotulos = np.concatenate([np.zeros(len(normais), dtype=int), np.ones(len(riscosos), dtype=int)])
    x_treino, x_teste, y_treino, y_teste = train_test_split(entradas, rotulos, test_size=0.25, random_state=42, stratify=rotulos)
