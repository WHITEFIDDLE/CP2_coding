"""Desafio 10: versão corrigida para comparação LOCAL com exploit.py.

Instalar: python -m pip install Flask mysql-connector-python
Executar: python app_seguro.py (pare antes o app vulnerável na porta 5000).

PowerShell, no mesmo terminal da execução:
    $env:MYSQL_USER = 'cp2_app'
    $env:MYSQL_PASSWORD = Read-Host 'Senha do usuário MySQL cp2_app'
    $env:API_KEY_ANA = python -c "import secrets; print(secrets.token_urlsafe(32))"
    $env:API_KEY_BRUNO = python -c "import secrets; print(secrets.token_urlsafe(32))"

As chaves são associadas aos IDs 1 e 2, NÃO a privilégios fixos.
O nivel_acesso é consultado no MySQL em cada DELETE. Os dois usuários devem
existir em seguranca.usuarios(id, nome, nivel_acesso, ...).
Não é necessário criar uma coluna senha nem uma coluna api_key.

Preparação pelo DBA/DBeaver (substitua a senha fictícia antes de executar):
CREATE USER 'cp2_app'@'%' IDENTIFIED BY 'SUBSTITUA_POR_SENHA_FORTE';
GRANT SELECT (id, nome, nivel_acesso), DELETE ON seguranca.usuarios TO 'cp2_app'@'%';
O host '%' atende ao Docker local; restrinja o host e a rede fora do laboratório.
Não conceda ALL PRIVILEGES nem utilize root para executar esta aplicação.

Teste sem credencial: python exploit.py --permitir-exclusao --uid 2
Teste de privilégio insuficiente (Bruno deve ter nivel_acesso < 5):
    curl.exe -i -X DELETE -H "X-API-Key: $env:API_KEY_BRUNO" http://127.0.0.1:5000/api/usuarios/1
Teste positivo, SOMENTE com um ID descartável (apaga de verdade):
    curl.exe -i -X DELETE -H "X-API-Key: $env:API_KEY_ANA" http://127.0.0.1:5000/api/usuarios/99

Auditoria: arquivo auditoria_desafio10.sqlite3 ao lado do script. Não é apagado
ao reiniciar. Registra tentativa antes da rota e conclusão depois da resposta.
Se a auditoria inicial falhar, a rota NÃO executa (503). Se falhar após commit,
retorna 503 e alerta no terminal; isso não desfaz uma exclusão já confirmada.
SQLite e MySQL não compartilham transação: uma tentativa sem conclusão exige
reconciliação. Em produção use outbox transacional e coleta/alertas externos.
Proteja o arquivo com permissões do SO; não o publique nem o inclua no Git.

RELATORIO_TESTE_ERRO=1 (padrão deste laboratório) simula falha interna para o
teste esperado de 500. Defina 0 para retornar um relatório real de contagem.

Controles corrigidos: SQL parametrizado; XSS escapado; autenticação; autorização;
erros genéricos; projeção de campos; headers; debug desligado; segredos externos;
auditoria e alertas locais; conexão com privilégios mínimos e fechamento seguro.
Ausências adicionadas: identificação/permissões e registros de segurança.

Busca pública de id/nome é uma decisão DIDÁTICA para preservar o exploit atual.
Em um sistema com cadastro privado, proteja também a busca com autenticação.
Não usa cookies de autenticação ou CORS; token somente no header X-API-Key.
HTTP e servidor Flask são apenas para loopback; produção exige HTTPS e WSGI.
...
def criar_app():
    ...
    # Sem query string, corpo, senha ou API key na auditoria.
    @app.before_request
    ...
    def tratar_erro(erro):
        if isinstance(erro, HTTPException) and erro.code < 500:
            ...
            return resposta
        # Não registra mensagens SQL, parâmetros ou traceback contendo segredos.
    ...
    def buscar():
        ...
        with banco() as (_, cur):
            # O f-string monta somente o VALOR, nunca a consulta.
    ...
    def perfil():
        ...
        # Template fixo e escape padrão; nunca adicionar |safe ao nome.
    ...
    def remover(uid):
        ...
        with banco() as (con, cur):
            # Bloqueia a linha até terminar a transação: privilégio lido e usado
            # na mesma transação. Não confia em nível enviado pelo cliente.
            ...
            # Somente após autorização consulta/altera o alvo.
    ...
    def relatorio():
        if os.environ.get('RELATORIO_TESTE_ERRO', '1') == '1':
            # Falha deliberada somente para o teste ANTES/DEPOIS do enunciado.
...
if __name__ == '__main__':
    ...
    # Impede o access log padrão de copiar query strings para o terminal.
